from fastapi import FastAPI, Depends, HTTPException, File, UploadFile, Body, Header
...
async def get_memed_token(user=Depends(get_current_user)):
    ...
    try:
        token = await memed.obter_token(
            ...
            cpf=normalizar_cpf(user.cpf) or "",
            crm=user.conselho_numero or user.numero_conselho or "",
            uf=user.conselho_uf or user.uf_conselho or "",
