...
interface InputFieldProps extends React.InputHTMLAttributes<HTMLInputElement> {
  ...
  required?: boolean;
  mask?: 'cpf' | 'cep' | 'phone';
  onCepFetched?: (data: any) => void;
}

export function InputField({ label, required, className, mask, onCepFetched, onChange, value, defaultValue, type, ...props }: InputFieldProps) {
  ...
  const handleChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (mask === 'cpf') rawValue = maskCPF(rawValue);
    else if (mask === 'cep') rawValue = maskCEP(rawValue);
    else if (mask === 'phone') rawValue = maskPhone(rawValue);

    // Provide the masked value to the actual event
    ...
    // CEP auto-fetch
    if (mask === 'cep' && rawValue.replace(/\D/g, '').length === 8 && onCepFetched) {
      try {
  ...
  const handleBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    ...
    if (mask === 'cpf') {
      if (!isValidCPF(String(displayValue))) setError('CPF inválido');
    } else if (type === 'email') {
      if (!isValidEmail(String(displayValue))) setError('E-mail inválido');
    }
  };
